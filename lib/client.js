window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		async function sendToAgent(report) {
			...
			const sendButton = [...textarea.closest("[data-composer-card]")?.querySelectorAll("button[aria-label]") ?? []].find((button) => !button.disabled && ["发送消息", "Send message"].includes(button.getAttribute("aria-label") ?? ""));
			if (sendButton !== void 0) sendButton.click();
		...
		*/
		function showErrorBar(block, source, error) {
			...
			sendButton.addEventListener("click", () => {
				sendToAgent(report).then((sent) => flash(sendButton, sent ? "已发送" : "已复制"));
			});
