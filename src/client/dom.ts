...
function isVisible(block: HTMLElement): boolean {
  return lazyObserver === undefined || visibleBlocks.has(block)
}

// --- render queue -----------------------------------------------------------

/** One queued render: a first render (needs the loading placeholder) or a refresh (theme flip). */
 ...
 */
function enqueueRender(block: HTMLElement, kind: 'initial' | 'refresh'): void {
  if (block.hasAttribute(ERROR_ATTR)) return
  if (kind === 'initial' && (block.hasAttribute(RENDERED_ATTR) || !pendingSources.has(block))) return
 ...
 * @param block - the `.md-code-block` element.
 */
function showLoading(block: HTMLElement): void {
  if (block.querySelector(`.${HOST_CLASS}`) !== null) return
...
export async function renderBlock(
  block: HTMLElement,
  source: string,
  env: MermaidRenderEnv,
  options: { loading?: boolean } = {},
): Promise<boolean> {
  if (block.hasAttribute(ERROR_ATTR)) return false
...
// --- error summary + copy / send-to-agent ------------------------------------
 ...
 */
export async function sendToAgent(report: string): Promise<boolean> {
  ...
  const sendButton = [...(card?.querySelectorAll<HTMLButtonElement>('button[aria-label]') ?? [])]
    .find(button => !button.disabled && ['发送消息', 'Send message'].includes(button.getAttribute('aria-label') ?? ''))
  if (sendButton !== undefined) {
    // Use the host's actual submit control when it exists. HTMLElement.click()
    // reaches React's onClick and avoids relying on a synthetic keyboard event.
    sendButton.click()
  } else {
    // During an ordinary running turn DSH deliberately replaces Send with Stop,
    // while Enter remains the supported queue/steer submission gesture.
...
export function showErrorBar(block: HTMLElement, source: string, error: unknown): void {
  ...
  sendButton.textContent = '发送给 Agent 修复'
  sendButton.addEventListener('click', () => {
    void sendToAgent(report).then(sent => flash(sendButton, sent ? '已发送' : '已复制'))
  })
 ...
 */
function svgNaturalSize(svg: Element): { w: number; h: number } {
...
export function openOverlay(block: HTMLElement): void {
  ...
  const visualSize = (): { w: number; h: number } => {
  ...
  const onWheel = (event: WheelEvent): void => {
    ...
    if (panning) return // don't zoom mid-pan
...
export function scan(env: MermaidRenderEnv): void {
  ...
  for (const block of document.querySelectorAll<HTMLElement>(CODE_BLOCK_SELECTOR)) {
    ...
    if (block.hasAttribute(RENDERED_ATTR)) {
      ...
      if (observer !== undefined) observer.observe(block)
      continue
    }
